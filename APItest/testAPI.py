...
def main():
    ...
    try:
        ...
        pretty_json = json.dumps(api_data, indent=4)
        # Print the formatted JSON string
        print(type(pretty_json), type(api_data)) """
    ...
    # --- 3. Process the data (Example: Extract a specific field) ---
        ...
        print(f"Current Time: {current_time}, {loc_timezone}")
        print(f"Current Temperature: {temperature}°C")
        print(f"Feels Like: {feels_like}°C")
        print(f"Humidity: {humidity}%")
        print(f"Wind Speed: {wind_speed} m/s")
        print(f"Wind Direction: {wind_direction} deg")
        print(f"Weather Condition: {main_weather} ({weather_description})")
        #==========================================
